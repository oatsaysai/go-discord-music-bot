...
func YoutubeFind(searchString string, v *VoiceInstance, m *discordgo.MessageCreate) (song_struct PkgSong, err error) { //(url, title, time string, err error)
	...
	client := &http.Client{
		Transport: &transport.APIKey{Key: viper.GetString("Youtube.APIKey")},
	}
	...
	ytClient := ytLib.Client{}

	vid, err := ytClient.GetVideo(fmt.Sprintf("https://www.youtube.com/watch?v=%s", audioId))
	if err != nil {
		panic(err)
	}

	respDown, err := ytClient.GetStream(vid, &vid.Formats[0])
	if err != nil {
		panic(err)
	}
